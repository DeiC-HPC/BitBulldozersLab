...
The job can be executed and adjusted in `run-singlenode.sh` and `run-multinode.sh`. Either job takes roughly 90 seconds each with the default parameters. There is a boolean to make the example very small and run in a few seconds in `MnO2.py`
